...
public class Wallhack
{
    public static void OnPlayerTransmit(CCheckTransmitInfo info, CCSPlayerController player)
    {
        ...
        {
            if (!Util.IsPlayerValid(entity.Key) || !Util.IsPlayerValid(player))
                continue;

            if (entity.Key.Team != player.Team &&
                player.Team != CsTeam.Spectator &&
                entity.Key.Team != CsTeam.Spectator)
            {
    ...
    public static HookResult OnPlayerDisconnect(EventPlayerDisconnect @event, GameEventInfo info)
    {
        ...
        if (!Globals.GlowData.TryGetValue(player!, out var glowData))
            return HookResult.Continue;
    ...
    public static HookResult OnPlayerChangeTeam(EventPlayerTeam @event, GameEventInfo info)
    {
        ...
        if (!glowData.GlowEnt.IsValid || !glowData.ModelRelay.IsValid)
            return HookResult.Continue;
